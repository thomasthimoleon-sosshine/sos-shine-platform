 ...
 *  COURRIER ANONYME
 * ─────────────────────────────────────────────────────────────────────────────
 *  Le formulaire vit ici pour pouvoir etre rendu a deux endroits sans etre
 *  ecrit deux fois : dans Mon compte (mode compact), et sur sa propre route
 *  /dashboard/courrier-anonyme, conservee pour les liens deja partages.
 */
...
export default function CourrierAnonyme({ compact = false }: { compact?: boolean }) {
  ...
  return (
    <div className={compact ? '' : 'max-w-3xl mx-auto'}>
      {/* En-tete : masque quand le formulaire est integre dans Mon compte,
          ou la carte porte deja le titre. */}
      {!compact && (
      <div className="mb-8">

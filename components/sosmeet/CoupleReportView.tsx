/**
 * SOS Meet Couple — l'affichage de la carte.
 * Un seul composant, utilisé par la page de démonstration ET par la vraie
 * carte des couples. C'est volontaire : la démo ne peut pas mentir sur le
 * rendu, puisqu'elle passe par le même code.
 */
...
function Observation({ o, accent }: { o: { source: string; titre: string; texte: string }; accent: string }) {
  return (
    <div className="rounded-2xl p-6" style={{ background: C.card, border: `1px solid ${C.line}`, borderLeft: `2px solid ${accent}` }}>
...
export default function CoupleReportView({
  report, prenomA, prenomB, demo = false,
}: { report: CoupleReport; prenomA: string; prenomB: string; demo?: boolean }) {
  const n = report.narrative
  const e = report.energetique
    ...
    <main className="min-h-screen relative" style={{ ...sans, background: C.ink, color: C.alabaster }}>
      ...
      <div className="relative max-w-2xl mx-auto px-6 py-20">
        ...
        {demo && (
          <div className="mt-6 mb-12 rounded-2xl px-6 py-5" style={{ background: 'rgba(155,27,46,0.10)', border: `1px solid ${C.garnet}` }}>
            <p className="text-[13.5px] leading-relaxed" style={{ color: C.alabaster }}>
              <b>Démonstration.</b> {prenomA} et {prenomB} n’existent pas. Leurs réponses ont été fabriquées pour montrer
              à quoi ressemble la lecture d’un couple. Le texte, les scores et la lecture énergétique sont en revanche
              produits par le vrai moteur, sans retouche, et affichés par la même page que les vrais couples.
            </p>
          </div>
        )}
        ...
        <h1 className="mt-4 mb-8" style={{ ...serif, fontWeight: 400, fontSize: 'clamp(2rem,6.4vw,3rem)', lineHeight: 1.06 }}>
          {prenomA} <span style={{ color: C.smoke2 }}>et</span> {prenomB}
        </h1>
        ...
        <div className="flex gap-5 mb-14 pb-10" style={{ borderBottom: `1px solid ${C.line}` }}>
          <Jauge label="Santé du lien" valeur={report.crossing.sante} />
          <Jauge label="Vous vous lisez juste" valeur={report.crossing.lucidite} />
        </div>
        ...
        {n.laOuCaBloque.length > 0 && (
          <Section titre="Là où ça bloque">
            <div className="flex flex-col gap-4">
              {n.laOuCaBloque.map((b, i) => {
                const f = report.crossing.findings.find(x => x.dimension === b.dimension)
                return (
                  <div key={i} className="rounded-2xl p-6" style={{ background: C.card, border: `1px solid ${C.line}`, borderTop: `2px solid ${C.garnet}` }}>
                    <div className="flex items-baseline justify-between mb-2.5 gap-3">
                      ...
                      {f && <span className="text-[10.5px] tracking-[0.16em] uppercase whitespace-nowrap" style={{ color: C.ember }}>{VERDICT_LABEL[f.verdict]}</span>}
                    </div>
        ...
        {e ? (
          <Section titre="Votre lecture énergétique">
            <p className="text-[15px] leading-relaxed mb-8" style={{ color: C.smoke }}>
              ...
              de l’attention. Elle ne diagnostique rien. Aucune des difficultés plus haut n’a été détectée par une
              position planétaire, elles viennent toutes de vos réponses.
            </p>
            ...
            <div className="flex flex-col gap-3.5 mb-10">
              {e.complementarites.map((o, i) => <Observation key={i} o={o} accent={C.garnet} />)}
            </div>

            <h3 className="mb-4" style={{ ...serif, fontWeight: 400, fontSize: '1.35rem' }}>Ce qui demande de l’attention</h3>
            <div className="flex flex-col gap-3.5 mb-10">
              {e.attentions.map((o, i) => <Observation key={i} o={o} accent={C.smoke2} />)}
            </div>
            ...
            <details className="rounded-2xl px-6 py-5" style={{ border: `1px solid ${C.line}` }}>
              ...
              <div className="mt-5 text-[14.5px] leading-relaxed" style={{ color: C.smoke }}>
                <p className="mb-2">Chemins de vie {e.detail.numerologie.cheminA} et {e.detail.numerologie.cheminB}, nombre du couple {e.detail.numerologie.nombreCouple}.</p>
                <p className="mb-2">
                ...
                <p className="mb-5">Profils {e.detail.humanDesign.a.profil} et {e.detail.humanDesign.b.profil}.</p>
        ...
        ) : report.energetiqueAbsente ? (
          <Section titre="Votre lecture énergétique">
            <div className="rounded-2xl p-6" style={{ background: C.card, border: `1px solid ${C.line}` }}>
              <p className="text-[15px] leading-relaxed mb-4" style={{ color: C.smoke }}>{report.energetiqueAbsente}</p>
              <Link href="/sos-meet/couple/duo" className="text-[13.5px] tracking-[0.08em] uppercase" style={{ color: C.ember }}>
                Ajouter nos dates de naissance →
              </Link>
            </div>
          </Section>
        ) : null}

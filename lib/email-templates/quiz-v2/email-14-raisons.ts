...
export function generateEmail14(vars: Vars): { subject: string; html: string } {
  ...
  const content = [
    ...
    p(`C'est justement pour ça que SOS Shine est sans engagement. Pour que tu ressentes par toi-même si cet endroit te parle vraiment. Si la voix qu'on y entend résonne avec ce que tu cherches. Si la méthode du déconditionnement parle à ton corps autant qu'à ton mental.`),
    goldDivider(),

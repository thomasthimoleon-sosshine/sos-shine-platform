...
export default function LandingAdminPage() {
  ...
  const loadSections = useCallback(async () => {
    ...
    if (!data || data.length === 0) {
      ...
      // Reload after seed
      const { data: seeded } = await supabase.from('landing_sections').select('*').eq('page_version', PAGE_VERSION).order('position', { ascending: true })
      if (seeded) {
      ...
      if (missingDefaults.length > 0) {
        ...
        const { data: refreshed } = await supabase.from('landing_sections').select('*').eq('page_version', PAGE_VERSION).order('position', { ascending: true })
        if (refreshed) {
  ...
  async function handleSave() {
    ...
    try {
      ...
      const { error: upsertErr } = await supabase
        .from('landing_sections')
        .upsert(rows, { onConflict: 'page_version,section_key' })
  ...
  function renderContentFields(sec: LandingSectionRow) {
    ...
    switch (key) {
      ...
      case 'nav':
        return (
          <>
            <TextField label="Texte connexion" value={c.login_label || ''} onChange={(v) => updateContent(key, 'login_label', v)} />
            <TextField label="Lien connexion" value={c.login_href || ''} onChange={(v) => updateContent(key, 'login_href', v)} />
            <TextField label="Texte CTA" value={c.cta_label || ''} onChange={(v) => updateContent(key, 'cta_label', v)} />
            <TextField label="Lien CTA" value={c.cta_href || ''} onChange={(v) => updateContent(key, 'cta_href', v)} />
          </>
        )
      ...
      case 'hero_v2':
        return (
          <>
            <TextField label="Badge" value={c.badge || ''} onChange={(v) => updateContent(key, 'badge', v)} />
            <TextAreaField label="Titre principal" value={c.title || ''} onChange={(v) => updateContent(key, 'title', v)} />
            <TextAreaField label="Description" value={c.description || ''} onChange={(v) => updateContent(key, 'description', v)} />
            <FileUpload label="Image hero" accept="image/*" folder="site"
              currentUrl={c.image_url || null}
              onUploaded={(url) => updateContent(key, 'image_url', url)}
              onRemoved={() => updateContent(key, 'image_url', '')} />
            <FileUpload label="Video hero" accept="video/*" folder="site"
              currentUrl={c.video_url || null}
              onUploaded={(url) => updateContent(key, 'video_url', url)}
              onRemoved={() => updateContent(key, 'video_url', '')} />
            <Separator label="Boutons" />
      ...
      case 'truth':
        return (
          <>
            <TextAreaField label="Titre" value={c.title || ''} onChange={(v) => updateContent(key, 'title', v)} />
            <TextAreaField label="Paragraphes (un par ligne vide)" rows={6}
              value={Array.isArray(c.paragraphs) ? c.paragraphs.join('\n\n') : ''}
              onChange={(v) => updateContent(key, 'paragraphs', v.split('\n\n').filter((p: string) => p.trim() !== ''))} />
            <TextAreaField label="Conclusion" value={c.conclusion || ''} onChange={(v) => updateContent(key, 'conclusion', v)} />
          </>
        )
      ...
      case 'julia_book':
        return (
          <>
            <TextAreaField label="Titre" value={c.title || ''} onChange={(v) => updateContent(key, 'title', v)} />
            <TextAreaField label="Intro auteur" value={c.author_intro || ''} rows={4} onChange={(v) => updateContent(key, 'author_intro', v)} />
            <TextAreaField label="Histoire auteur" value={c.author_story || ''} rows={4} onChange={(v) => updateContent(key, 'author_story', v)} />
            <TextAreaField label="Texte plateforme" value={c.platform_text || ''} rows={4} onChange={(v) => updateContent(key, 'platform_text', v)} />
            <TextAreaField label="Promesse" value={c.promise || ''} rows={3} onChange={(v) => updateContent(key, 'promise', v)} />
            <TextAreaField label="Signature" value={c.signature || ''} onChange={(v) => updateContent(key, 'signature', v)} />
            <FileUpload label="Photo auteur" accept="image/*" folder="team"
              currentUrl={c.author_image || null}
              onUploaded={(url) => updateContent(key, 'author_image', url)}
              onRemoved={() => updateContent(key, 'author_image', '')} />
            <FileUpload label="Image du livre" accept="image/*" folder="site"
              currentUrl={c.book_image || null}
              onUploaded={(url) => updateContent(key, 'book_image', url)}
              onRemoved={() => updateContent(key, 'book_image', '')} />
            <TextField label="Lien du livre" value={c.book_url || ''} onChange={(v) => updateContent(key, 'book_url', v)} />
            <Separator label="Equipe" />
            <TextField label="Intro equipe" value={c.team_intro || ''} onChange={(v) => updateContent(key, 'team_intro', v)} />
            <TextAreaField label="Description equipe" value={c.team_description || ''} onChange={(v) => updateContent(key, 'team_description', v)} />
            {(c.team_members || []).map((m: { name: string; role: string; image: string }, i: number) => (
              <div key={i} className="rounded-lg p-4 space-y-3" style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid var(--dark-border)' }}>
                <div className="flex items-center justify-between">
                  <p className="text-xs font-semibold uppercase tracking-wider" style={{ color: 'var(--text-muted)' }}>Membre {i + 1}</p>
                  <button type="button" onClick={() => removeArrayItem(key, 'team_members', i)}
                    className="text-xs px-2 py-1 rounded cursor-pointer" style={{ color: '#FF6B6B' }}>Supprimer</button>
                </div>
                <TextField label="Nom" value={m.name || ''} onChange={(v) => updateArrayItem(key, 'team_members', i, 'name', v)} />
                <TextField label="Role" value={m.role || ''} onChange={(v) => updateArrayItem(key, 'team_members', i, 'role', v)} />
                <FileUpload label="Photo" accept="image/*" folder="team"
                  currentUrl={m.image || null}
                  onUploaded={(url) => updateArrayItem(key, 'team_members', i, 'image', url)}
                  onRemoved={() => updateArrayItem(key, 'team_members', i, 'image', '')} />
              </div>
            ))}
            <button type="button" onClick={() => addArrayItem(key, 'team_members', { name: '', role: '', image: '' })}
              className="text-sm px-4 py-2 rounded-lg cursor-pointer" style={{ color: '#74C0FC', border: '1px dashed #74C0FC' }}>
              + Ajouter un membre
            </button>
          </>
      ...
      case 'member_stories':
        return (
          <>
            ...
            <TextField label="Sous-titre" value={c.subtitle || ''} onChange={(v) => updateContent(key, 'subtitle', v)} />
            <Separator label="Temoignages" />
            {(c.items || []).map((item: { quote: string }, i: number) => (
              <div key={i} className="rounded-lg p-4 space-y-3" style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid var(--dark-border)' }}>
                <div className="flex items-center justify-between">
                  <p className="text-xs font-semibold uppercase tracking-wider" style={{ color: 'var(--text-muted)' }}>Temoignage {i + 1}</p>
                  <button type="button" onClick={() => removeArrayItem(key, 'items', i)}
            ...
            <button type="button" onClick={() => addArrayItem(key, 'items', { quote: '' })}
              className="text-sm px-4 py-2 rounded-lg cursor-pointer" style={{ color: '#74C0FC', border: '1px dashed #74C0FC' }}>
              + Ajouter un temoignage
            </button>
          </>
      ...
      /* ────────────── encyclopedia_v2 ────────────── */
      case 'encyclopedia_v2':
        return (
          <>
            <TextField label="Badge" value={c.badge || ''} onChange={(v) => updateContent(key, 'badge', v)} />
            <TextAreaField label="Titre" value={c.title || ''} onChange={(v) => updateContent(key, 'title', v)} />
            <TextAreaField label="Description" value={c.description || ''} onChange={(v) => updateContent(key, 'description', v)} />
            <TextField label="Placeholder recherche" value={c.search_placeholder || ''} onChange={(v) => updateContent(key, 'search_placeholder', v)} />
            <TextAreaField label="Tags (un par ligne)" rows={6}
              value={Array.isArray(c.tags) ? c.tags.join('\n') : ''}
              onChange={(v) => updateContent(key, 'tags', v.split('\n').filter((t: string) => t.trim() !== ''))} />
            <TextField label="Label 'Et plus'" value={c.more_label || ''} onChange={(v) => updateContent(key, 'more_label', v)} />
            <TextField label="Texte bouton" value={c.button_label || ''} onChange={(v) => updateContent(key, 'button_label', v)} />
            <TextField label="Lien bouton" value={c.button_href || ''} onChange={(v) => updateContent(key, 'button_href', v)} />
          </>
        )
      ...
      case 'parcours':
        return (
          <>
            ...
            <TextAreaField label="Titre" value={c.title || ''} onChange={(v) => updateContent(key, 'title', v)} />
            <Separator label="Etapes" />
            {(c.steps || []).map((step: { num: string; title: string; description: string; color: string }, i: number) => (
              <div key={i} className="rounded-lg p-4 space-y-3" style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid var(--dark-border)' }}>
                <div className="flex items-center justify-between">
                  <p className="text-xs font-semibold uppercase tracking-wider" style={{ color: 'var(--text-muted)' }}>Etape {i + 1}</p>
                  <button type="button" onClick={() => removeArrayItem(key, 'steps', i)}
                    className="text-xs px-2 py-1 rounded cursor-pointer" style={{ color: '#FF6B6B' }}>Supprimer</button>
                </div>
                <TextField label="Numero" value={step.num || ''} onChange={(v) => updateArrayItem(key, 'steps', i, 'num', v)} />
                <TextField label="Titre" value={step.title || ''} onChange={(v) => updateArrayItem(key, 'steps', i, 'title', v)} />
                <TextAreaField label="Description" value={step.description || ''} onChange={(v) => updateArrayItem(key, 'steps', i, 'description', v)} />
                <ColorField label="Couleur" value={step.color || ''} onChange={(v) => updateArrayItem(key, 'steps', i, 'color', v)} />
              </div>
            ))}
            <button type="button" onClick={() => addArrayItem(key, 'steps', { num: String((c.steps?.length || 0) + 1).padStart(2, '0'), title: '', description: '', color: '#74C0FC' })}
              className="text-sm px-4 py-2 rounded-lg cursor-pointer" style={{ color: '#74C0FC', border: '1px dashed #74C0FC' }}>
              + Ajouter une etape
            </button>
          </>
      ...
      case 'community_v2':
        return (
          <>
            ...
            <TextAreaField label="Titre" value={c.title || ''} onChange={(v) => updateContent(key, 'title', v)} />
            <TextAreaField label="Description" value={c.description || ''} onChange={(v) => updateContent(key, 'description', v)} />
            <TextAreaField label="Paragraphe" value={c.paragraph || ''} onChange={(v) => updateContent(key, 'paragraph', v)} />
            <TextField label="Conclusion" value={c.closing || ''} onChange={(v) => updateContent(key, 'closing', v)} />
            <Separator label="Blocs" />
            {(c.blocks || []).map((block: { icon: string; title: string; description: string }, i: number) => (
              <div key={i} className="rounded-lg p-4 space-y-3" style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid var(--dark-border)' }}>
                <div className="flex items-center justify-between">
                  <p className="text-xs font-semibold uppercase tracking-wider" style={{ color: 'var(--text-muted)' }}>Bloc {i + 1}</p>
                  <button type="button" onClick={() => removeArrayItem(key, 'blocks', i)}
                ...
                <TextField label="Icone (emoji)" value={block.icon || ''} onChange={(v) => updateArrayItem(key, 'blocks', i, 'icon', v)} />
                <TextField label="Titre" value={block.title || ''} onChange={(v) => updateArrayItem(key, 'blocks', i, 'title', v)} />
                <TextAreaField label="Description" value={block.description || ''} onChange={(v) => updateArrayItem(key, 'blocks', i, 'description', v)} />
              </div>
            ))}
            <button type="button" onClick={() => addArrayItem(key, 'blocks', { icon: '', title: '', description: '' })}
              className="text-sm px-4 py-2 rounded-lg cursor-pointer" style={{ color: '#74C0FC', border: '1px dashed #74C0FC' }}>
              + Ajouter un bloc
            </button>
          </>
      ...
      case 'not_us':
        return (
          <>
            <TextAreaField label="Titre" value={c.title || ''} onChange={(v) => updateContent(key, 'title', v)} />
            <TextAreaField label="Citation" value={c.quote || ''} rows={4} onChange={(v) => updateContent(key, 'quote', v)} />
            <TextField label="Auteur" value={c.author || ''} onChange={(v) => updateContent(key, 'author', v)} />
            <TextAreaField label="Avertissement" value={c.disclaimer || ''} onChange={(v) => updateContent(key, 'disclaimer', v)} />
            <TextAreaField label="Description" value={c.description || ''} rows={4} onChange={(v) => updateContent(key, 'description', v)} />
            <TextField label="Conclusion" value={c.closing || ''} onChange={(v) => updateContent(key, 'closing', v)} />
          </>
        )
      ...
      case 'pricing_v2':
        return (
          <>
            ...
            <TextField label="Sous-titre" value={c.subtitle || ''} onChange={(v) => updateContent(key, 'subtitle', v)} />
            <Separator label="Plans tarifaires" />
            {(c.plans || []).map((plan: { name: string; price: string; period: string; description: string; button_label: string; button_href: string; highlight: boolean; badge: string; features: string[] }, i: number) => (
              <div key={i} className="rounded-lg p-4 space-y-3" style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid var(--dark-border)' }}>
                <div className="flex items-center justify-between">
                  <p className="text-xs font-semibold uppercase tracking-wider" style={{ color: 'var(--text-muted)' }}>Plan {i + 1}</p>
                  <button type="button" onClick={() => removeArrayItem(key, 'plans', i)}
                ...
                <TextField label="Nom" value={plan.name || ''} onChange={(v) => updateArrayItem(key, 'plans', i, 'name', v)} />
                <TextField label="Prix" value={plan.price || ''} onChange={(v) => updateArrayItem(key, 'plans', i, 'price', v)} />
                <TextField label="Periode" value={plan.period || ''} onChange={(v) => updateArrayItem(key, 'plans', i, 'period', v)} />
                <TextField label="Description" value={plan.description || ''} onChange={(v) => updateArrayItem(key, 'plans', i, 'description', v)} />
                <TextField label="Texte bouton" value={plan.button_label || ''} onChange={(v) => updateArrayItem(key, 'plans', i, 'button_label', v)} />
                <TextField label="Lien bouton" value={plan.button_href || ''} onChange={(v) => updateArrayItem(key, 'plans', i, 'button_href', v)} />
                <CheckboxField label="Mis en avant" checked={!!plan.highlight} onChange={(v) => updateArrayItem(key, 'plans', i, 'highlight', v)} />
                <TextField label="Badge" value={plan.badge || ''} onChange={(v) => updateArrayItem(key, 'plans', i, 'badge', v)} />
                <TextAreaField label="Features (une par ligne)" rows={5}
                  value={Array.isArray(plan.features) ? plan.features.join('\n') : ''}
                  onChange={(v) => updateArrayItem(key, 'plans', i, 'features', v.split('\n').filter((l: string) => l.trim() !== ''))} />
            ...
            <button type="button" onClick={() => addArrayItem(key, 'plans', { name: '', price: '', period: '/mois', description: '', button_label: '', button_href: '/rejoindre', highlight: false, badge: '', features: [] })}
              className="text-sm px-4 py-2 rounded-lg cursor-pointer" style={{ color: '#74C0FC', border: '1px dashed #74C0FC' }}>
              + Ajouter un plan
            </button>
            <Separator label="Texte de pied" />
            <TextField label="Titre pied" value={c.footer_title || ''} onChange={(v) => updateContent(key, 'footer_title', v)} />
            <TextAreaField label="Texte pied" value={c.footer_text || ''} onChange={(v) => updateContent(key, 'footer_text', v)} />
            <TextAreaField label="Citation pied" value={c.footer_quote || ''} onChange={(v) => updateContent(key, 'footer_quote', v)} />
          </>
        )
      ...
      case 'for_who':
        return (
          <>
            <TextField label="Titre" value={c.title || ''} onChange={(v) => updateContent(key, 'title', v)} />
            <TextAreaField label="Points positifs (un par ligne)" rows={5}
              value={Array.isArray(c.positive) ? c.positive.join('\n') : ''}
              onChange={(v) => updateContent(key, 'positive', v.split('\n').filter((l: string) => l.trim() !== ''))} />
            <TextField label="Titre negatif" value={c.negative_title || ''} onChange={(v) => updateContent(key, 'negative_title', v)} />
            <TextAreaField label="Points negatifs (un par ligne)" rows={3}
              value={Array.isArray(c.negative) ? c.negative.join('\n') : ''}
              onChange={(v) => updateContent(key, 'negative', v.split('\n').filter((l: string) => l.trim() !== ''))} />
          </>
        )
      ...
      case 'cta_final':
        return (
          <>
            ...
            <TextField label="Texte bouton" value={c.button_label || ''} onChange={(v) => updateContent(key, 'button_label', v)} />
            <TextField label="Lien bouton" value={c.button_href || ''} onChange={(v) => updateContent(key, 'button_href', v)} />
            <TextField label="Details (prix, etc)" value={c.details || ''} onChange={(v) => updateContent(key, 'details', v)} />
          </>
        )
      ...
      case 'footer_v2':
        return (
          <>
            <TextField label="Texte logo" value={c.logo_text || ''} onChange={(v) => updateContent(key, 'logo_text', v)} />
            <TextField label="Copyright" value={c.copyright || ''} onChange={(v) => updateContent(key, 'copyright', v)} />
            <Separator label="Liens" />
            {(c.links || []).map((link: { label: string; href: string }, i: number) => (
              <div key={i} className="rounded-lg p-4 space-y-3" style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid var(--dark-border)' }}>
                <div className="flex items-center justify-between">
                  <p className="text-xs font-semibold uppercase tracking-wider" style={{ color: 'var(--text-muted)' }}>Lien {i + 1}</p>
                ...
                <TextField label="Libelle" value={link.label || ''} onChange={(v) => updateArrayItem(key, 'links', i, 'label', v)} />
                <TextField label="Lien" value={link.href || ''} onChange={(v) => updateArrayItem(key, 'links', i, 'href', v)} />
              </div>
            ))}
            <button type="button" onClick={() => addArrayItem(key, 'links', { label: '', href: '' })}
              className="text-sm px-4 py-2 rounded-lg cursor-pointer" style={{ color: '#74C0FC', border: '1px dashed #74C0FC' }}>
            ...
            <Separator label="Reseaux sociaux" />
            {(c.socials || []).map((s: { label: string; href: string }, i: number) => (
              <div key={i} className="rounded-lg p-4 space-y-3" style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid var(--dark-border)' }}>
                <div className="flex items-center justify-between">
                  <p className="text-xs font-semibold uppercase tracking-wider" style={{ color: 'var(--text-muted)' }}>Reseau {i + 1}</p>
                  <button type="button" onClick={() => removeArrayItem(key, 'socials', i)}
                    className="text-xs px-2 py-1 rounded cursor-pointer" style={{ color: '#FF6B6B' }}>Supprimer</button>
                </div>
                <TextField label="Nom" value={s.label || ''} onChange={(v) => updateArrayItem(key, 'socials', i, 'label', v)} />
                <TextField label="URL" value={s.href || ''} onChange={(v) => updateArrayItem(key, 'socials', i, 'href', v)} />
              </div>
            ))}
            <button type="button" onClick={() => addArrayItem(key, 'socials', { label: '', href: '' })}
              className="text-sm px-4 py-2 rounded-lg cursor-pointer" style={{ color: '#74C0FC', border: '1px dashed #74C0FC' }}>
    ...
    <div className="max-w-4xl mx-auto space-y-4">
      ...
      <div className="flex items-center justify-between sticky top-0 z-20 py-4 -mx-4 px-4" style={{ background: 'var(--dark)' }}>
        ...
        <div className="flex items-center gap-3">
          ...
          <a href={`/?preview=${PAGE_VERSION}`} target="_blank" rel="noopener noreferrer"

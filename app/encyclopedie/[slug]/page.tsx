...
export default function PublicDouleurDetailPage() {
  ...
  function hasContent(step: typeof steps[0]): boolean {
    const media = getStepMedia(step.num)
    const hasMedia = !!(media.video || media.audio || media.pdf || media.image)
    if (step.num === 3) return hasMedia || !!douleur?.exercise_content
    return hasMedia
  }
    ...
    <main className="min-h-screen" style={{ background: 'var(--dark)' }}>
      ...
      <div className="max-w-4xl mx-auto px-6 md:px-8 py-12 space-y-8">
        ...
        <div className="rounded-2xl p-6 sm:p-8" style={{ background: `${currentStep.color}06`, border: `1px solid ${currentStep.color}15` }}>
          ...
          {/* Locked content area — show available media types as locked previews */}
          {(() => {
            const media = getStepMedia(currentStep.num)
            const hasAny = media.video || media.audio || media.pdf || media.image || (currentStep.num === 3 && douleur?.exercise_content)

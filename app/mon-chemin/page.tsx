...
function MonCheminContent() {
  ...
  useEffect(() => {
    async function init() {
      ...
      // Resolve protocol slug: URL param → sessionStorage → Supabase (user_id) → Supabase (email scores)
      ...
      // Supabase fallback by user_id: check top_protocol_slug first, then compute from scores
      if (!found && uid) {
        ...
        if (latestResponse?.top_protocol_slug) {
          const { data } = await (supabase as any)
            .from('protocols').select('*').eq('slug', latestResponse.top_protocol_slug).single()
          found = data || null
        } else if (latestResponse?.scores) {
          const { data: allProtocols } = await (supabase as any).from('protocols').select('*')
          if (allProtocols) {
            const matched = (allProtocols as Protocol[])
              .map(p => ({ ...p, matchScore: calculateMatchScores(latestResponse.scores, p.dimension_weights) }))
      ...
      // Legacy fallback by email (for responses before user_id linking)
      if (!found && user?.email) {
        const { data: responses } = await (supabase as any)
          .from('quiz_v2_responses')
          .select('top_protocol_slug, scores')
  ...
  function saveProgress(step: 1 | 2 | 3, done = false) {
    ...
    // localStorage (fast, backwards compat)
    try {

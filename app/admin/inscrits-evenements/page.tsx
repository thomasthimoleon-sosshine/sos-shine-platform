...
export default function InscritsEvenementsAdmin() {
  ...
  useEffect(() => {
    ...
    async function load() {
      const [eventsRes, resvRes] = await Promise.all([
        supabase
          .from('physical_events')
          .select('id, title, event_date, location_name, is_free, is_published')
          .order('event_date', { ascending: false }),
        supabase
          .from('ceremonie_reservations')
          .select('id, prenom, nom, email, status, created_at, paid_at, event_id')
          .order('created_at', { ascending: false }),
      ])
      if (eventsRes.data) setEvents(eventsRes.data as PhysicalEvent[])
      if (resvRes.data) setReservations(resvRes.data as Reservation[])
      setLoading(false)
    }
  ...
  const countForEvent = (eventId: string) =>
    reservations.filter(r => r.event_id === eventId).length

  const visibleReservations = reservations.filter(r => {
    if (selectedEventId && r.event_id !== selectedEventId) return false
    if (filter !== 'all' && r.status !== filter) return false
    const q = search.toLowerCase()
    if (q && !r.prenom.toLowerCase().includes(q) && !r.nom.toLowerCase().includes(q) && !r.email.toLowerCase().includes(q)) return false
    return true
  })

  const botEntries = visibleReservations.filter(isBot)

  const selectedEvent = selectedEventId ? events.find(e => e.id === selectedEventId) : null
  const totalVisible = visibleReservations.length
  const paidVisible = visibleReservations.filter(r => r.status === 'paid').length
  ...
  const copyEmails = () => {
    const emails = visibleReservations.filter(r => r.status === 'paid' || r.status === 'pending').map(r => r.email).join(', ')
  ...
  return (
    <div className="max-w-5xl mx-auto py-8 px-4">
      ...
      {/* Event selector */}
      <div className="mb-6">
        <p className="text-xs uppercase tracking-widest text-[var(--text-muted)] mb-3">Filtrer par événement</p>
        <div className="flex flex-wrap gap-2">
          <button
            onClick={() => setSelectedEventId(null)}
            className="px-4 py-2 rounded-xl text-sm font-medium transition-all"
            style={{
              background: !selectedEventId ? 'rgba(201,169,97,0.15)' : 'var(--surface-card)',
              border: `1px solid ${!selectedEventId ? 'rgba(201,169,97,0.4)' : 'var(--border)'}`,
              color: !selectedEventId ? 'var(--brand)' : 'var(--text-secondary)',
            }}>
            Tous ({reservations.length})
          </button>
          {events.map(ev => {
            const count = countForEvent(ev.id)
            const active = selectedEventId === ev.id
            return (
              <button
                key={ev.id}
                onClick={() => setSelectedEventId(active ? null : ev.id)}
                className="px-4 py-2 rounded-xl text-sm font-medium transition-all text-left"
                style={{
                  background: active ? 'rgba(201,169,97,0.15)' : 'var(--surface-card)',
                  border: `1px solid ${active ? 'rgba(201,169,97,0.4)' : 'var(--border)'}`,
      ...
      {selectedEvent && (
        <div className="rounded-xl p-4 mb-6 flex items-center gap-4"
          ...
          <div className="flex-1">
            <p className="text-sm font-semibold text-[var(--brand)]">{selectedEvent.title}</p>
            <p className="text-xs text-[var(--text-muted)] mt-0.5">
              {formatEventDate(selectedEvent.event_date) || 'Date non définie'}
              {selectedEvent.location_name && ` · ${selectedEvent.location_name}`}
              {selectedEvent.is_free ? ' · Gratuit' : ' · Payant'}
            </p>
          </div>
          <div className="text-right">
            <p className="text-2xl font-light text-[var(--text-primary)]">{countForEvent(selectedEvent.id)}</p>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6">
        {[
          { label: 'Inscrits affichés', value: totalVisible, color: 'var(--text-primary)' },
          { label: 'Confirmés/Payés', value: paidVisible, color: '#55EFC4' },
          { label: 'En attente', value: visibleReservations.filter(r => r.status === 'pending').length, color: '#C9A961' },
          { label: 'Entrées suspectes', value: botEntries.length, color: '#ef4444' },
        ].map(s => (
        ...
        <div className="rounded-xl p-4 mb-6 flex items-center justify-between gap-4"
          ...
          <div>
            <p className="text-sm font-medium" style={{ color: '#ef4444' }}>
              🤖 {botEntries.length} entrée{botEntries.length > 1 ? 's' : ''} suspecte{botEntries.length > 1 ? 's' : ''} détectée{botEntries.length > 1 ? 's' : ''}
            </p>
            <p className="text-xs mt-0.5 text-[var(--text-muted)]">Noms ou emails au format bot</p>
          </div>
          <button
            ...
            {cleaning ? 'Suppression...' : confirmClean ? '⚠️ Confirmer' : `🗑 Supprimer (${botEntries.length})`}
          </button>
        </div>
      )}
      ...
      {visibleReservations.length === 0 ? (
        <div className="text-center py-16 text-[var(--text-muted)]">
          {reservations.length === 0 ? 'Aucune inscription pour le moment.' : 'Aucun résultat pour ces filtres.'}
        </div>
      ) : (
            ...
            <table className="w-full text-sm">
              <thead>
                <tr style={{ background: 'var(--surface-card)', borderBottom: '1px solid var(--border)' }}>
                  {['Prénom', 'Nom', 'Email', !selectedEventId ? 'Événement' : null, 'Statut', 'Inscrit le', ''].filter(Boolean).map(h => (
              ...
              <tbody>
                {visibleReservations.map((r, i) => {
                  ...
                  const bot = isBot(r)
                  const eventName = r.event_id ? events.find(e => e.id === r.event_id)?.title : null
                  return (
                    <tr key={r.id}
                      style={{
                      ...
                      <td className="px-4 py-3 text-[var(--text-secondary)] max-w-[120px] truncate">{r.nom}</td>
                      <td className="px-4 py-3">
                        <a href={`mailto:${r.email}`} className="hover:underline" style={{ color: bot ? '#ef4444' : 'var(--brand)' }}>{r.email}</a>
                      </td>
                      {!selectedEventId && (
                        <td className="px-4 py-3 text-xs text-[var(--text-muted)] max-w-[140px] truncate">
                          {eventName || <span className="opacity-40">—</span>}
                        </td>
                      )}
                      <td className="px-4 py-3">
              ...
              return (
                <div key={r.id} className="p-4 flex items-start gap-3"
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {eventName && <p className="text-xs text-[var(--text-muted)] mb-1">{eventName}</p>}
      ...
      <p className="text-xs text-[var(--text-muted)] mt-4 text-center">
        {visibleReservations.length} inscription{visibleReservations.length !== 1 ? 's' : ''} affichée{visibleReservations.length !== 1 ? 's' : ''}
      </p>
    </div>
  )
}
